...
#define DELAY_TO_OFF_V 10       // Задержка после выкл. света в ванной до выкл. вентилятора (сек.)
#define DELAY_TO_OFF_T 20       // Задержка после выкл. света в туалете до выкл. вентилятора (сек.)
...
void main(void)
{
	...
	while (1) { 
        ...
        if ((last_off == 1 || last_off == 2) && !time_cnt_res && time_cnt >= DELAY_TO_ON) {
            time_cnt = 0;
            time_cnt_res = 1;
        } 
